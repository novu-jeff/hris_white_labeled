...
{
    /**
     * Stream timelog captured image through the app so the browser uses our HTTPS domain.
     * Avoids "Not Secure" / mixed content when S3 endpoint has certificate issues.
     */
    public function show(Request $request, string $path): StreamedResponse
    {
        ...
        $employeeNo = $user && !empty($user->employee_no) ? trim((string) $user->employee_no) : null;
        if ($employeeNo === null) {
            abort(403);
        }

        // Employee may only view images under their own folder (path is e.g. ni-067/filename.png).
        $prefix = strtolower($employeeNo) . '/';
